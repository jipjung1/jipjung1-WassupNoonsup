...
# searchterm = '수지'
def crawl(name):
    url = "https://www.google.com/search?q=" + name + "&source=lnms&tbm=isch"
    # chrom webdriver 사용하여 브라우저를 가져온다.
    browser = webdriver.Chrome('/Users/joon2/Downloads/chromedriver')
    browser.get(url)
    ...
    for x in browser.find_elements_by_xpath('//div[contains(@class,"rg_meta")]'):
        ...
        try:
            ...
            File = open(os.path.join(name, name + "_" + str(counter) + "." + imgtype), "wb")
            File.write(raw_img)
            File.close()
...
def main():
    namelist = ['슈퍼주니어 최시원', '은혁', '인피니트 남우현', '박유천', '슈퍼주니어 예성', '슈퍼주니어 려욱', '슈퍼주니어 신동',
                '송일국', '이서진', '모모', '나연', '사나']
